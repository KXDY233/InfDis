...
def influence_discount(k, dict, neighbor_dict):
    seeds = []
    te = {}
    fe = {}
    yta = {}
...
if __name__ == '__main__':
    ...
    for pro in p_list:
        ...
        seeds, time_log = influence_discount(k, subset_dict, neighbor_dict)


        seed_path = data_path + "seeds_"+ str(p)
        joblib.dump(seeds, seed_path)

        outfile = "./result/time_" + data + "_InfDis_" + str(p) + ".txt"
        time_results = np.array(time_log)
        np.savetxt(outfile, time_results, fmt='%.3f', delimiter='\t')
