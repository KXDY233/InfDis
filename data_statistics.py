...
def influence_spread_computation_IC_new(dict, seeds, unionset, earlystopping, rounds, p):
    influence = []
...
def influence_spread_computation_IC_Mu(dict, seeds, unionset, earlystopping, num_mcmc, p):
    pool = multiprocessing.Pool()
    n = multiprocessing.cpu_count()
    results = []
    sub = int(num_mcmc / n)
    for i in range(n):
        result = pool.apply_async(influence_spread_computation_IC_new,
                                  args=(dict, seeds, unionset, earlystopping, sub, p,))
...
if __name__ == '__main__':
    ...
    print("number of groups: ", len_list)

    #unionset = dict2listset(subset_dict)

    temp = []
    for li in list(subset_dict.keys()):
        temp = temp + subset_dict[li]

    print("number of nodes: ", len(set(temp)))
    
    
    print("max degree: ", len(subset_dict[0]))
    
    print("average degree: ", len(temp)/len_list)

    qq = []
    for key in list(neighbor_dict.keys()):
        qq.append(len(neighbor_dict[key]))

    print("max q: ", np.max(qq))
    print("avg q: ", np.mean(qq))
